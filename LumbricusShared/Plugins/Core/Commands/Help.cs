        ...
        public override void HandleCommand(IrcLine line, Nick nick, Channel channel)
        {
            try {
                ...
                if (helpUriSetting == null) {
                    conn.SendPrivmsg(nick.Name, String.Format("Oh dear… I'm afraid I don't seem to be able to answer that right now, {0}!", nick.DisplayNick));
                    return;
                }
                if (nick.Account != null && isOp(nick)) {
                    ...
                    conn.SendPrivmsg(target, String.Format("Hi, @{0}. Main help is at {1}. You also have access to these op-only commands: !seen, !baninfo, !botban, !restart", nick.DisplayNick, helpUriSetting.Value));
                } else {
                    conn.SendPrivmsg(nick.Name, String.Format("Hi, {0}. Help is at {1}.", nick.DisplayNick, helpUriSetting.Value));
                    if (channel != null && !channel.AllowCommandsInChannel) {
